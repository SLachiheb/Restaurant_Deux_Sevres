...
import BackgroundSplit from './../BackgroundSplit/BackgroundSplit';
import BlurGlass from './../BlurGlass/BlurGlass';

/*
Type : Function Component.
Return : A detailed restaurant page with the name, address, telephone number, description and rating system of the restaurant. The presentation of the restaurant images is done by slideshow.
*/
...
const RestaurantDetails = props => {
  ...
  const ratingHandler = async e => {
    console.log(e.target.value, props.restaurant.id);
    const response = await fetch('/api/new-rating', {
      method: 'POST',
      body: JSON.stringify({
  ...
  const items = props.restaurant.images.map((img, index) => {
    return {
      src: img,
      altText: `Slide ${index}`,
      caption: '',
    ...
    <BackgroundSplit>
      ...
      <BlurGlass>
        <div className={classes.restaurantDetails_container}>
          <div className={classes.restaurantDetails_item__left}>
            <h1 className={classes.restaurantDetails_name}>
              {props.restaurant.name}
            </h1>
            <div className={classes.restaurantDetails_location}>
              <FontAwesomeIcon
                className={classes.icon_location}
                icon={faMapMarkerAlt}
              />
              <p className={classes.address}>{props.restaurant.address}</p>
            </div>
            <div className={classes.restaurantDetails_contact}>
              <FontAwesomeIcon
                className={classes.icon_phone}
                icon={faPhoneVolume}
              />
              <p>{props.restaurant.phone}</p>
            </div>

            <div className={classes.restaurantDetails_score}>
              <FontAwesomeIcon className={classes.icon_score} icon={faStar} />
              <p className={classes.score}>
                {router.query.rating} ({router.query.nbRatings})
              </p>
            </div>

            <div className={classes.description_scroll}>
              ...
              {props.restaurant.images.map((img, index) => {
                return (
                  <div className='each-slide' key={index}>
                    <img src={img} alt={`${props.restaurant.name}`}></img>
                  </div>
                );
              })}
            </Slide>
